CMD:acmds(playerid)
{
	...
	strcat(line3, ""WHITE_E"Administrator Commands:"LB2_E"\n\
 	/aduty /a /h /aa /togooc /o /goto /sendto /gethere /freeze /unfreeze /revive /spec /slap\n\
 	/caps /peject /astats /rstats /acuff /auncuff /jetpack /getip /aka /akaip /jail /unjail\n\
	/kick /ban /unban /respawnsapd /respawnsags /respawnsamd /respawnsana /respawnjobs\n\
	/reports /respond /dr /vmodels /vehname /apv /aveh /gotoveh /getveh /respawnveh /respawnrad");

	strcat(line3, "\n\n"WHITE_E"Senior Admin Commands:"LB2_E"\n\
 	/sethp /clearreports /afix /setskin /akill /cnn /cd /settime /setweather\n\
    /rjail /rwarn /setam /gotoco /gotohouse /gotobisnis /gotodoor /gotolocker /gotogs");

	strcat(line3, "\n\n"WHITE_E"Lead Admin Commands:"LB2_E"\n\
	/rban /banip /unbanip /reloadweap /resetweap /sethbe\n\
 	/createdoor /editdoor");
	...
	strcat(line3, "\n"BLUE_E"RT:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"RT:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:hcmds(playerid)
{
	...
	/kick /slap /caps /acuff /auncuff /reports /respond /dr");

	strcat(line3, "\n\n"WHITE_E"Senior Helper Commands:"LB2_E"\n\
 	/spec /peject /astats /rstats /jetpack\n\
    /jail /unjail");
	...
	strcat(line3, "\n"BLUE_E"RT:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Permanently!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"RT:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:aduty(playerid, params[])
{
	...
	if(!pData[playerid][pAdminDuty])
    {
		if(pData[playerid][pAdmin] > 0)
		{
			...
			SendStaffMessage(COLOR_RED, "* %s telah on duty admin.", pData[playerid][pAdminname]);
		}
		else
		{
			...
			pData[playerid][pAdminDuty] = 1;
			SetPlayerName(playerid, pData[playerid][pAdminname]);
			SendStaffMessage(COLOR_RED, "* %s telah on helper duty.", pData[playerid][pAdminname]);
		}
    }
    else
    {
        ...
        SendStaffMessage(COLOR_RED, "* %s telah off admin duty.", pData[playerid][pAdminname]);
    }
	return 1;
}
...
CMD:aa(playerid, params[]) 
{
    ...
    if(sscanf(params,"s[225]",text))
        return Usage(playerid, "/aa [text]");

    SendClientMessageToAllEx(COLOR_RED,"Admin "YELLOW_E"%s: "LG_E"%s", pData[playerid][pAdminname], ColouredText(text));
    return 1;
}
...
CMD:rstats(playerid, params[])
{
	...
	{
	    Usage(playerid, "/rstats <player name>");
 		return 1;
 	}
...
CMD:rwarn(playerid, params[])
{
	...
	if(sscanf(params, "s[24]s[50]", player, tmp))
		return Usage(playerid, "/rwarn <name> <reason>");
...
CMD:rjail(playerid, params[])
{
	...
	if(sscanf(params, "s[24]ds[50]", player, datez, tmp))
		return Usage(playerid, "/rjail <name> <time in minutes)> <reason>");
...
CMD:setjob1(playerid, params[])
{
	...
	if(jobid < 0 || jobid > 7)
        return Error(playerid, "Invalid ID. 0 - 7.");
...
CMD:setweather(playerid, params[])
{
    ...
    SendClientMessageToAllEx(COLOR_RIKO,"WEATHER: {FFFFFF}Admin {FF0000}%s {FFFFFF}Telah mengganti Weather ke ID: {FF0000}%d", pData[playerid][pAdminname], weatherid);
...
CMD:rban(playerid, params[])
{
	...
	{
	    Usage(playerid, "/rban <ban name> <time in days (0 for permanent ban)> <reason>");
...
CMD:serverpanel(playerid, params[])
{
	...
	if(pData[playerid][pAdmin] >= 6)
	{
		format(string, sizeof(string), "Lock Server\nChange Hostname\nMapname\nWeburl\nLanguage\nUnlock Server");
	    ShowPlayerDialog(playerid, SERVER_PANEL, DIALOG_STYLE_LIST, "Server Control Panel",string,"Select","Cancel");
	}
	else SEM(playerid, "ERROR: You don't have the privilege to use this command!");
	return 1;
}
CMD:gmx(playerid, params[])
{
        ...
        if(pData[playerid][pAdmin] >= 6)
	    {
	    	if(CurGMX == 0)
	    	{
	    		format(String, sizeof(String), "GMX:{FFFF00} %s {FFFFFF}has initiated a server restart, it will occur in the next {FFFF00}30 seconds.", pData[playerid][pAdminname]);
...
CMD:ans(playerid,params[])
{
	...
	new id, answer[128];
	if (sscanf(params, "ds[128]", id, answer))
	    return Usage(playerid, "/ans [playerid] [answer]");
	    
	SendStaffMessage(COLOR_RED, "ANSWER: %s {FF0000}- %s", answer, pData[playerid][pAdminname]);
	Servers(id, "ANSWER: %s. By: %s", answer, pData[playerid][pAdminname]);
	AskActive[id] = 0;
	return 1;
}
