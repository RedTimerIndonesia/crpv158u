...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == SERVER_PANEL)
	{
	    if(response)
	    {
	        ...
	        if(listitem == 0)
	        {
	            format(string, sizeof(string), "{ffffff}Please input the password server to Lock Server\nServer Password: {00ff00}(input below)");
	            ShowPlayerDialog(playerid, LOCK_SERVER, DIALOG_STYLE_INPUT, "Server Lock",string,"Lock","Cancel");
	        }
	        if(listitem == 1)
	        {
	            format(string, sizeof(string), "{ffffff}Please input the hostname to change server hostname\nHostname: {00ff00}(input below)");
	            ShowPlayerDialog(playerid, CHANGE_HOSTNAME, DIALOG_STYLE_INPUT, "Server Hostname",string,"Change","Cancel");
	        }
	        if(listitem == 2)
	        {
	            format(string, sizeof(string), "{ffffff}Please input the mapname to change server mapname\nMapname: {00ff00}(input below)");
	            ShowPlayerDialog(playerid, MAPNAME_DIALOG, DIALOG_STYLE_INPUT, "Server Mapname",string,"Change","Cancel");
	        }
	        if(listitem == 3)
	        {
	            format(string, sizeof(string), "{ffffff}Please input the weburl to change server weburl\nWeburl: {00ff00}(input below)");
	            ShowPlayerDialog(playerid, WEBURL_DIALOG, DIALOG_STYLE_INPUT, "Server Weburl",string,"Change","Cancel");
	        }
	        if(listitem == 4)
	        {
	            format(string, sizeof(string), "{ffffff}Please input the language to change server language\nLanguage: {00ff00}(input below)");
	            ShowPlayerDialog(playerid, LANGUAGE_DIALOG, DIALOG_STYLE_INPUT, "Server Language",string,"Change","Cancel");
	        }
	        if(listitem == 5)
	        {
	            ...
	            {
	                ServerLocked = false;
	                format(string, sizeof(string), "password 0");
	                SendRconCommand(string);
	                format(string, sizeof(string), "AdmWarn: %s has unlocked the server", pData[playerid][pAdminname]);
	                SendStaffMessage(COLOR_LRED, string);
	            }
	            else SEM(playerid, "ERROR: Server not locked");
	            return 1;
	        }
	    }
	    return 1;
	}
	if(dialogid == LOCK_SERVER)
	{
	    new string[256];
	    if(response)
	    {
			if(ServerLocked == false)
			{
			    ...
			    SendRconCommand(string);
			    format(string, sizeof(string), "AdmWarn: %s has locked the server", pData[playerid][pAdminname]);
       			SendStaffMessage(COLOR_LRED, string);
			}
			else SEM(playerid, "ERROR: Server already currently locked");
			return 1;
	    }
	    return 1;
	}
	if(dialogid == CHANGE_HOSTNAME)
	{
	    new string[256];
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "AdmWarn: %s has change server hostname", pData[playerid][pAdminname]);
         	SendStaffMessage(COLOR_LRED, string);
	    }
		return 1;
	}
	if(dialogid == MAPNAME_DIALOG)
	{
	    new string[256];
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "AdmWarn: %s has change server mapname", pData[playerid][pAdminname]);
         	SendStaffMessage(COLOR_LRED, string);
	    }
	    return 1;
	}
	if(dialogid == WEBURL_DIALOG)
	{
	    new string[256];
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "AdmWarn: %s has change server weburl", pData[playerid][pAdminname]);
         	SendStaffMessage(COLOR_LRED, string);
	    }
	    return 1;
	}
	if(dialogid == LANGUAGE_DIALOG)
	{
	    ...
	    if(response)
	    {
	        ...
	        SendRconCommand(string);
	        format(string, sizeof(string), "AdmWarn: %s has change server language", pData[playerid][pAdminname]);
         	SendStaffMessage(COLOR_LRED, string);
	    }
	    return 1;
	}
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			...
			{
				new str[3500];
				strcat(str, ""LG_E"PLAYER: /help /afk /drag /undrag /pay /stats /items /frisk /use /give /idcard /drivelic\n");
