/*==============================================*/
/*  Gamemode Author	: Dandy Bagus Prasetyo	   	*/
/*  Developed by	: az						*/
/*	Ext. Developer	: FakhriYT   				*/
/*==============================================*/

/*==============================================*/
/*	Credits to Christian Malik for:				*/
/*	===============================				*/
/*	Command '/lt' '/rt' '/ft' '/up' '/dn' etc.	*/
/*	Command '/mark' '/gotomark'					*/
/*==============================================*/
...
/*	CRP v1.5.3u2 Racism													*/
/* 	======================												*/
/*	[CHANGE] Command '/ar' to '/respond'								*/
/*	[REMOVE] '/tr' Command												*/
/*	[REMOVE] ASGH Building												*/
/*																		*/
/*======================================================================*/
/*																		*/
/*	CRP v1.5.7u Mode Baru												*/
/*	========================											*/
/*	[ADD] Player CMD '/ask' '/clear'									*/
/*	[ADD] Admin CMD '/flip' '/entercar' '/gentercar' 					*/
/*					'/setvw' '/setint' '/gmx' '/gotohq' '/gotoflint'	*/
/*																		*/
/*	[IMPROVED] CMD aduty[add aliases '/aod']							*/
/*	[ADD] Boombox System												*/
/*	[ADD] Quiz System													*/
/*	[IMPROVED] CMD '/setweather'										*/
/*	[IMPROVED] HUD HBE at '/settings > HUD HBE Mode'					*/
/*																		*/
/*======================================================================*/
/*																		*/	
/*	CRP v1.5.8u Mode Baru												*/
/*	============================										*/
/*	[ADD] Interpolate Cam and Music at login							*/
/*	[CHANGE] Usage Prefix to KEGUNAAN									*/
/*	[CHANGE] Map Teleporter Only function while admin/helper on duty	*/
/*																		*/
/*======================================================================*/
...
#define flashtime 120 //milliseconds for the flash, larger number = slower flash
...
SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		...
		case 6: // Login 1
        {
            SetPlayerPos(playerid, 1300.0640, -1130.1039, 23.8264);
			TogglePlayerControllable(playerid, 0);
            InterpolateCameraPos(playerid, 801.657226, -1144.682373, 26.994699, 1268.322998, -1144.056762, 28.954261, 40000);
			InterpolateCameraLookAt(playerid, 806.638977, -1144.769165, 26.576974, 1263.332031, -1144.037109, 28.654720, 1000);
        }
        case 7: // Login 2
        {
			SetPlayerPos(playerid, 1610.8751, -1680.1879, 26.0342);
            TogglePlayerControllable(playerid, 0);
			InterpolateCameraPos(playerid, 1638.745361, -1079.667358, 127.142982, 1600.351806, -1673.165039, 38.453361, 40000);
			InterpolateCameraLookAt(playerid, 1638.116455, -1084.494506, 126.001426, 1600.701416, -1668.208129, 39.006763, 2000);
        }
		case 8: // Market
		{
			SetPlayerPos(playerid, 1634.7699, -1120.2974, 55.1167);
            TogglePlayerControllable(playerid, 0);
			InterpolateCameraPos(playerid, 812.479858, -1144.597778, 27.618888, 1338.014648, -1143.766357, 27.188869, 60000);
			InterpolateCameraLookAt(playerid, 817.479370, -1144.639648, 27.562004, 1333.016967, -1143.657226, 27.084594, 36);
		}

	}
	return 1;
}
SetPlayerJoinMusic(playerid)
{
	new randmusic = Random(1,7);
	switch(randmusic)
	{
		case 1: // Arcade Game Jingle
		{
		    PlayerPlaySound(playerid, 1062, 0.0, 0.0, 0.0);
  		}
  		case 2: // Rock and Electro Theme song
		{
			PlayerPlaySound(playerid, 1097, 0.0, 0.0, 0.0);
  		}
  		case 3: // Bike School music
		{
		    PlayerPlaySound(playerid, 1185, 0.0, 0.0, 0.0);
  		}
		case 4: // Driving School music
		{
			PlayerPlaySound(playerid, 1183, 0.0, 0.0, 0.0);
		}
		case 5: // Arcade game jingle 2
		{
			PlayerPlaySound(playerid, 1076, 0.0, 0.0, 0.0);
		}
		case 6: // Pilot school music
		{
			PlayerPlaySound(playerid, 1187, 0.0 , 0.0, 0.0);
		}
		case 7: // Driving School Music
		{
...
// ask
new AskTime[MAX_PLAYERS], AskActive[MAX_PLAYERS];
...
#include "Extras/COLOR.pwn"
#include "Extras/TEXTDRAW.pwn"
...
public OnGameModeInit()
{
	...
	new string[256];
	format(string, sizeof(string), "language %s",SERVER_LANGUAGE);
	SendRconCommand(string);
	format(string, sizeof(string), "hostname %s",SERVER_NAME);
	SendRconCommand(string);
	format(string, sizeof(string), "mapname %s",SERVER_MAP);
	SendRconCommand(string);
	format(string, sizeof(string), "weburl %s",SERVER_WEBURL);
	SendRconCommand(string);
	format(string, sizeof(string), "password %s",SERVER_NORMAL);
	SendRconCommand(string);
	SetGameModeText(GAMEMODE_NAME" v"MODE_VERSION);
	ServerLocked = false;
...
public OnPlayerCommandPerformed(playerid, cmd[], params[], result, flags)
{
    if (result == -1)
    {
        Error(playerid, "Unknown Command! /help for more info.");
        return 0;
    }
...
public OnPlayerConnect(playerid)
{
	...
	PlayAudioStreamForPlayer(playerid, "http://mcdn.podbean.com/mf/web/b9xhhr/ToluSahundulan.mp3");
	return 1;
}
